"""JULIUS Database Manager - Pure SQLite, no external dependencies."""
...
class JuliusDatabase:
    ...
    def _init_tables(self):
        ...
        # Revenue summary table for quick stats
        cursor.execute("""
        ...
        cursor.execute("INSERT OR IGNORE INTO revenue_summary (key, value, updated_at) VALUES ('total_revenue', 0, ?)", (datetime.utcnow().isoformat(),))
    ...
    def release_escrow(self, escrow_id: str, proof_hash: str) -> bool:
        ...
        cursor.execute("""
            UPDATE escrows 
            SET status = 'released', released_at = ?, delivery_proof_hash = ?
            WHERE id = ?
    ...
    def _add_revenue(self, tx_type: str, amount_usd: float, complexity: float, multiplier: float, destination: str):
        """Internal method to add revenue transaction."""
        conn = self._get_connection()
        cursor = conn.cursor()
        ...
        cursor.execute("""
            ...
            (id, transaction_type, amount_usd, complexity, scaling_multiplier, destination, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    def add_revenue(self, tx_type: str, amount_usd: float, complexity: float = 1.0, destination: str = ""):
        """Add revenue transaction (public method)."""
        multiplier = 1.5 ** complexity  # Scaling per problem solved
